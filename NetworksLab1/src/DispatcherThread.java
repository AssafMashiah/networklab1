...
public class DispatcherThread implements Runnable {
	...
	public synchronized void run() {
		...
		HttpResposeParser response = null;
		try {
	 ...
	 */
	private HttpResposeParser generateNormalResponse(HttpRequestParser request)
	{
		HttpResposeParser response;
		...
		try
		{
			...
			response = new HttpResposeParser(HttpResponseCode.OK);
	 ...
	 */
	private HttpResposeParser generateTraceResponse(HttpRequestParser request)
	{
		HttpResposeParser response = new HttpResposeParser(HttpResponseCode.OK);
		response.SetHttpVersion(request.GetHttpVersion());
	 ...
	 */
	private HttpResposeParser generateHttpErrorResponse(HttpResponseCode code,
			HttpVersion version) {
		HttpResposeParser generatedResponse = new HttpResposeParser(code);
	...
	private HttpRequestParser parseClientRequestHeader(BufferedReader in)
		...
		int contentLength = httpHeadersParser(lines, newRequest);
	 ...
	 */
	private int httpHeadersParser(String[] lines, HttpRequestParser parser)
			throws HttpRequestParsingException {
		int contentLength = 0;
		...
		for (int i = 1; i < lines.length; i++) {
			...
			parser.AddHeader(new HttpHeader(headerName, headerValue));
